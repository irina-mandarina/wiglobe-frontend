...
<template>
    <div class="rounded-2xl my-4 shadow-lg w-3/4 mx-auto border-khaki border-2 text-center delay-[1000ms] details-box bg-white">
        <div v-if="!editMode">
        ...
        <div v-else>
            <div v-if="detailType === 'gender'" class="w-fit mx-auto">
                ...
                <input @click="setValue('FEMALE')" v-model="newValue" name="gender" value="FEMALE" type="radio" class="border rounded-full px-2 my-4 focus:outline-none"/>

                <i class="fa fa-male p-2 ml-6"/>
                <input @click="setValue('MALE')" v-model="newValue" name="gender" value="MALE"   type="radio" class="border rounded-full px-2 my-4 focus:outline-none"/>

                <i class="fa fa-user p-2 ml-6"/>
                <input @click="setValue('OTHER')" v-model="newValue" name="gender" value="OTHER"  type="radio" class="border rounded-full px-2 my-4 focus:outline-none"/>
            </div>

            <p v-else-if="detailType === 'birthdate'" class="text-center p-6">
                Born on <input type="date" v-model="newValue" />
                <br />
                <span v-if="newValue">{{ parseInt(( new Date().getTime() - new Date(newValue).getTime()) / (1000*60*60*24*365)) }}</span>
                <span v-else> _ </span> 
                years old
            </p>
            ...
            <p v-else-if="detailType === 'residence'" class="text-center p-6">
                From 
                <DestinationSearchBar @choose-destination="setValue" />
            </p>
